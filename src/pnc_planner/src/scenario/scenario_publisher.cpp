...
namespace pnc_planner::scenario {
  ...
  public:
    ScenarioPublisher()
      : Node("scenario_publisher") {
      const std::string scenario_file_path = this->declare_parameter<std::string>("scenario_file", "");

      scenario_data_ =
          ScenarioLoader::LoadFromFile(scenario_file_path);
      ...
      publisher_timer_ = this->create_wall_timer(
        ...
        [this]() { this->TryPublishRoute(); }
      );

      RCLCPP_INFO(
        this->get_logger(),
        "Loaded scenario '%s' from '%s' with %zu route points",
        scenario_data_.name.c_str(),
        scenario_file_path.c_str(),
        scenario_data_.route_points.size());
    }
  ...
  void ScenarioPublisher::TryPublishRoute() {
    ...
    route_path.header.frame_id = scenario_data_.frame_id;

    route_path.poses.reserve(
      scenario_data_.route_points.size());

    for (const auto &route_point: scenario_data_.route_points) {
    ...
    RCLCPP_INFO(
      this->get_logger(),
      "Published scenario '%s' to /routing_path",
      scenario_data_.name.c_str());
  }
} // namespace pnc_planner::scenario
...
int main(int argc, char **argv) {
  ...
  try {
    const auto node =
        std::make_shared<pnc_planner::scenario::ScenarioPublisher>();

    rclcpp::spin(node);
  } catch (const std::exception &e) {
    RCLCPP_FATAL(
      rclcpp::get_logger("scenario_publisher"),
      "%s",
      e.what());
